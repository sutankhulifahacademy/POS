...
export default function QRISPayment({ amount, description, onSuccess, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4">
      <div className="bg-[#2A1015] gold-border rounded-lg max-w-md w-full p-6" data-testid="qris-modal">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-serif-luxury text-2xl text-[#F4C842]">Pembayaran QRIS</h3>
        ...
        {error ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs text-[#C4A484]">Admin: tambahkan MIDTRANS_SERVER_KEY di /app/backend/.env kemudian restart backend.</p>
          </div>
        ) : loading ? (
          <div className="text-center py-16">
            <Loader2 className="animate-spin mx-auto text-[#F4C842]" size={40} />
            <p className="text-sm text-[#C4A484] mt-4">Membuat QR...</p>
          </div>
        ) : (
          <div className="text-center">
            <p className="text-sm text-[#C4A484]">Total Pembayaran</p>
            <p className="font-serif-luxury text-3xl text-[#F4C842] mb-4">{formatIDR(amount)}</p>
            {qr?.qr_image && <img src={qr.qr_image} alt="QRIS" className="mx-auto w-64 h-64 bg-white p-3 rounded-md" data-testid="qris-image" />}
            <p className="text-xs text-[#C4A484] mt-4">{qr?.order_id}</p>
            <div className="mt-4 py-3 border-t border-dashed border-[rgba(244,200,66,0.2)]">
              {status === "pending" ? (
            ...
            <p className="text-xs text-[#C4A484] mt-2 italic">Scan dengan aplikasi e-wallet apapun (GoPay, OVO, Dana, ShopeePay, dll.)</p>
          </div>
        )}
      </div>
    </div>
