  ...
  return (
    <div>
      <PageHeader title="Purchase Order" subtitle="Buat pesanan ke supplier, terima barang, stok otomatis bertambah" actions={
        <button onClick={() => setShowForm(true)} data-testid="add-po-btn" className="flex items-center gap-2 bg-[#D4AF37] text-[#050505] px-5 py-2.5 rounded-md text-sm font-semibold uppercase tracking-wider hover:bg-[#FFD700] transition-colors">
      ...
      <div className="p-8">
        <div className="bg-[#111] gold-border rounded-lg overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {orders.length === 0 && <tr><td colSpan={6} className="px-6 py-12 text-center text-[#A39B8B]">Belum ada PO</td></tr>}
              {orders.map((o) => (
                <tr key={o.id} className="border-b border-[rgba(212,175,55,0.08)] last:border-0 hover:bg-[#1A1A1A] transition-colors" data-testid={`po-row-${o.id}`}>
                  <td className="px-6 py-3 text-sm text-[#D4AF37] cursor-pointer" onClick={() => setDetail(o)}>{o.po_no}</td>
                  <td className="px-6 py-3 text-sm text-[#FDFBF7]">{o.supplier_name}</td>
                  <td className="px-6 py-3 text-xs text-[#A39B8B]">{new Date(o.created_at).toLocaleString("id-ID")}</td>
                  ...
                  <td className="px-6 py-3 text-right text-sm text-[#FDFBF7]">{formatIDR(o.total)}</td>
                    ...
                    <div className="flex justify-end gap-2">
                      {o.status === "draft" && (
                        <>
                          ...
                          <button onClick={() => remove(o.id)} className="text-[#A39B8B] hover:text-[#8B0000]"><Trash2 size={14} strokeWidth={1.5} /></button>
                        </>
                      )}
                    </div>
                  </td>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
          <div onClick={(e) => e.stopPropagation()} className="bg-[#0A0A0A] gold-border rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-[rgba(212,175,55,0.15)] flex items-center justify-between">
              <h2 className="font-serif-luxury text-2xl text-[#FDFBF7]">Buat Purchase Order</h2>
              <button onClick={() => setShowForm(false)} className="text-[#A39B8B] hover:text-[#FDFBF7]"><X size={20} /></button>
            </div>
            <form onSubmit={submit} className="p-6 space-y-4">
              <div>
                <label className="text-xs uppercase tracking-widest text-[#A39B8B] mb-1 block">Supplier</label>
                <select required value={supplierId} onChange={(e) => setSupplierId(e.target.value)} className="w-full bg-[#0A0A0A] border border-[rgba(212,175,55,0.2)] rounded-md px-3 py-2 text-[#FDFBF7]" data-testid="po-supplier">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs uppercase tracking-widest text-[#A39B8B]">Item Barang</label>
                ...
                <div className="space-y-2">
                  {items.map((it, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                      ...
                      }} className="col-span-6 bg-[#0A0A0A] border border-[rgba(212,175,55,0.2)] rounded-md px-2 py-2 text-sm text-[#FDFBF7]">
                        <option value="">Pilih produk</option>
                        {products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                      </select>
                      <input type="number" value={it.quantity} onChange={(e) => updateLine(idx, { quantity: e.target.value })} placeholder="Qty" className="col-span-2 bg-[#0A0A0A] border border-[rgba(212,175,55,0.2)] rounded-md px-2 py-2 text-sm text-[#FDFBF7]" />
                      <input type="number" value={it.cost} onChange={(e) => updateLine(idx, { cost: e.target.value })} placeholder="Harga modal" className="col-span-3 bg-[#0A0A0A] border border-[rgba(212,175,55,0.2)] rounded-md px-2 py-2 text-sm text-[#FDFBF7]" />
              ...
              <div className="flex justify-between items-center pt-4 border-t border-[rgba(212,175,55,0.15)]">
                <p className="text-sm text-[#A39B8B]">Total Estimasi</p>
              ...
              <div className="flex gap-3">
                ...
                <button type="submit" data-testid="po-submit" className="flex-1 bg-[#D4AF37] text-[#050505] py-2.5 rounded-md text-sm font-semibold uppercase tracking-widest hover:bg-[#FFD700] transition-colors">Simpan PO</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {detail && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={() => setDetail(null)}>
          <div onClick={(e) => e.stopPropagation()} className="bg-[#0A0A0A] gold-border rounded-lg max-w-lg w-full p-8">
            ...
            <p className="text-xs text-[#A39B8B] mb-4">{detail.supplier_name} · {new Date(detail.created_at).toLocaleString("id-ID")}</p>
              ...
              <tbody>
                {detail.items.map((i, idx) => (
                  <tr key={idx} className="border-b border-[rgba(212,175,55,0.08)]">
                    ...
                    <td className="py-2 text-right text-[#A39B8B]">{i.quantity}</td>
                    <td className="py-2 text-right text-[#A39B8B]">{formatIDR(i.cost)}</td>
                    <td className="py-2 text-right text-[#FDFBF7]">{formatIDR(i.quantity * i.cost)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
