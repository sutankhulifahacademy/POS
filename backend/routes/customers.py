...
async def create_item(body: MODEL, user=Depends(require_permission("customers", "create"))):
    d = body.model_dump()
    d["id"] = new_id()
...
async def update_item(item_id: str, body: MODEL, user=Depends(require_permission("customers", "update"))):
    d = body.model_dump()
...
async def delete_item(item_id: str, user=Depends(require_permission("customers", "delete"))):
